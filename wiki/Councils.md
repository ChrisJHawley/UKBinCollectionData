...
### Gateshead
```commandline
python collect_data.py GatesheadCouncil https://www.gateshead.gov.uk/article/3150/Bin-collection-day-checker -s -p "XXXX XXX" -n XX
```
Additional parameters:
- `-s` - skip get URL
- `-p` - postcode
...
### Gravesham
```commandline
python collect_data.py GraveshamBoroughCouncil https://my.gravesham.gov.uk -s -p "XXXX XXX" -n XX
```
Additional parameters:
- `-s` - skip get URL
- `-p` - postcode
...
Note: Pass your postcode and house number in their respective parameters, or alternatively a UPRN found via [FindMyAddress](https://www.findmyaddress.co.uk/search).
...
```commandline
python collect_data.py HinckleyandBosworthBoroughCouncil https://www.hinckley-bosworth.gov.uk -s -p "XXXX XXX" -n XX
```
Additional parameters:
- `-s` - skip get URL
- `-p` - postcode
...
Note: Pass your postcode and house number/name.
...
### Isle of Wight
```commandline
python collect_data.py IsleOfWightCouncil https://digitalservices.iow.gov.uk/wasteday -s -p "XXXX XXX" -n XX -w http://HOST:PORT/
```
Additional parameters:
- `-s` - skip get URL
- `-p` - postcode
...
Note: Pass your postcode and house number/name in their respective parameters.
...
Note: Pass your property's PID (labelled UPRN here) on its own, or a postcode and house number to search instead.
...
### Orkney Islands
```commandline
python collect_data.py OrkneyIslandsCouncil https://www.orkney.gov.uk -s -n XX
```
Additional parameters:
- `-s` - skip get URL
- `-n` - house number

Note: Pass a street, area, or island name (e.g. 'Kirkwall') in the house number parameter. Search at https://www.orkney.gov.uk/mybins to find your area.
